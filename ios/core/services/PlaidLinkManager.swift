...
final class PlaidLinkManager: ObservableObject {
    ...
    func prepareLinkSession() async {
        ...
        do {
            ...
            state = .error("Unable to initialize bank connection. Please try again.")
        }
    }
    ...
    func sendTokenToBackend() async -> Bool {
        ...
        do {
            ...
            state = .error("Failed to finalize connection. Please try again.")
            return false
        }
    }
    ...
    /// Base URL for the Trim backend API.
    /// Update this to match your deployment (e.g., production domain).
    private static let backendBaseURL = "http://localhost:3001/api/plaid"
    ...
    private func fetchLinkToken() async throws -> String {
        guard let url = URL(string: "\(Self.backendBaseURL)/create-link-token") else {
        ...
        let body = ["userId": "trim-user-\(UUID().uuidString.prefix(8))"]
    ...
    private func exchangePublicToken(_ token: String) async throws {
        guard let url = URL(string: "\(Self.backendBaseURL)/exchange-token") else {
            throw URLError(.badURL)
        }
